testUtilWuForNonCentWithXX <- function(sizeArray,
                                                  sd,
                                                  mu,
                                                  numberOfExperiment)
{
  cat("\nTesting \'tests_105_wuForNonCentWithXX\' \n")
  ...
  for(indexSize in 1:length(sizeArray))
  {
    ...
    title("Wu2009 For NonCentered sample with Original XX \nRenewal",sub = subtitle)
    graphics.off()
  }
  cat("\n End of wuForNonCentWithXX\n")
