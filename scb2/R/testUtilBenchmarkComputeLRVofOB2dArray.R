...
testUtilBenchmarkComputeLRVofOB2dArray <- function(size,
                                                   psi,
                                                   sigma,
                                                   mean)
{
  cat("\nTesting \'tests_93_testUtilBenchmarkComputeLRVofOB2dArray\' \n")
  cat("Test parameters:", "\n")
  cat("size =", size, "\n")
  cat("sigma =", sigma, "\n")
  cat("psi =", psi, "\n")
  ...
  fileName <- "BenchmarkComputeLRVofOB2dArray"
  ...
  for(index in 1:length(size))
  {
    ...
    original <- createOriginalMA1(size = size[index],
                                  ...
                                  sigma = sigma,
                                  mean = mean)
    res <- microbenchmark(computeLRVofOB2dArray(original = original,batchSize = batchSize),
                          times = 100L)
    ...
    title(main = "Median time ComputeLRVofOB2dArray",sub = subtitle)
    ...
    title(main = "ComputeLRVofOB2dArray Maximum and minimum time",sub = subtitle)
    graphics.off()
  }
  cat("End of test BenchMark testUtilBenchmarkComputeLRVofOB2dArray","\n")
