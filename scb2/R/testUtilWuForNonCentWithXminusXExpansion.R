testUtilWuForNonCentWithXminusXExpansion <- function(sizeArray,
                                                sd,
                                                mu,
                                                numberOfExperiment)
{
  cat("\nTesting \'tests_108_wuForNonCentWithXminusXExpansion\' \n")
  ...
  fileName <- "wuForNonCentWithXminusXExpansion"
  ...
  {
    originalMatrix[,columnIndex]<-createOriginalXminusX(size = max(sizeArray),
                                                        mu = mu,
                                                        sd = sd)
  }
  ...
  for(indexSize in 1:length(sizeArray))
  {
    ...
    title("Wu2009 For NonCentered sample with Original XminusX Expansion",sub = subtitle)
    graphics.off()
  }
  cat("\n End of wuForNonCentWithXminusXExpansion\n")
