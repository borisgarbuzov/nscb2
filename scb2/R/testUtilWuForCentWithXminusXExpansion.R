testUtilWuForCentWithXminusXExpansion <- function(sizeArray,
                                             sd,
                                             mu,
                                             numberOfExperiment)
{
  cat("\nTesting \'tests_104_wuForCentWithXminusXExpansion\' \n")
  ...
  fileName <- "wuForCentWithXminusXExpansion"
  ...
  for(indexSize in 1:length(sizeArray))
  {
    ...
    title("Wu2009 For Centered sample with Original XminusX Expansion",sub = subtitle)
    graphics.off()
  }

  cat("\n End of wuForCentWithXminusXExpansion\n")
