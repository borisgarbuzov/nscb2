...
testUtilBenchmarkEstimateWuForNonCenterdSample <- function(sizeArray,
                                                          psi,
                                                          sigma,
                                                          mean)
{
  cat("\nTesting \'tests_95_BenchmarkEstimateWuForNonCenterdSample\' \n")
  cat("Test parameters:", "\n")
  ...
  fileName <- "BenchmarkWuForNonCenterdSample"
  ...
  for(index in 1:length(sizeArray))
  {
    original <- createOriginalMA1(size = sizeArray[index],
                                  ...
                                  sigma = sigma,
                                  mean = mean)
    res <- microbenchmark(estimateLRVWu2009ForNonCenteredSample(sample = original) ,
                          times = 100L)
    ...
    title(main = "Median time EstimateWuNonCenteredSample ",sub = subtitle)
    ...
    title(main = "EstimateWuNonCenteredSample Maximum and minimum time",sub = subtitle)
    graphics.off()
  }

  cat("End of test BenchMark EstimateWuForNonCenterdSample","\n")
