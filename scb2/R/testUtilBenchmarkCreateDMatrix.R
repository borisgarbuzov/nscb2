...
testUtilBenchmarkCreateDMatrix <- function(sizeArray,
                                           psi,
                                           sigma,
                                           mean)
{
  cat("\nTesting \'tests_85_testUtilBenchmarkCreateDMatrix\' \n")
  ...
  for(index in 1:length(sizeArray))
  {
    ...
    title(main = "Median time Create DMatrix ",sub = subtitle)
    graphics.off()
    ...
    title(main = "CreateDMatrix Maximum and minimum time",sub = subtitle)
    graphics.off()
  }


  cat("End of test BenchmarkCreateDMatrix","\n")
