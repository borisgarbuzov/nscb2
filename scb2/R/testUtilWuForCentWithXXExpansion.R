testUtilWuForCentWithXXExpansion <- function(sizeArray,
                                            sd,
                                            mu,
                                            numberOfExperiment)
{
  cat("\nTesting \'tests_103_WuForCentWithXXExpansion\' \n")
  ...
  fileName <- "wuForCentWithXXExpansion"
  ...
  for(indexSize in 1:length(sizeArray))
  {
    ...
    title("Wu2009 For Centered sample with Original XX Expansion",sub = subtitle)
    graphics.off()
  }
  cat("\n End of WuForCentWithXXExpansion\n")
