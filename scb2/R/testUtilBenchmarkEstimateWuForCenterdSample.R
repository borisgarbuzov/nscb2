...
testUtilBenchmarkEstimateWuForCenterdSample <- function(sizeArray,
                                                           psi,
                                                           sigma,
                                                           mean)
{
  cat("\nTesting \'tests_96_BenchmarkEstimateWuForCenterdSample\' \n")
  cat("Test parameters:", "\n")
  ...
  fileName <- "BenchmarkEstimateWuForCenterdSample"
  ...
  for(index in 1:length(sizeArray))
  {
    original <- createOriginalMA1(size = sizeArray[index],
                                  ...
                                  sigma = sigma,
                                  mean = mean)
    res <- microbenchmark(estimateLRVWu2009ForCenteredSample(sample = original) ,
                          times = 100L)
    ...
    title(main = "Median time EstimateWuCenteredSample ",sub = subtitle)
    ...
    title(main = "EstimateWuCenteredSample Maximum and minimum time",sub = subtitle)
    graphics.off()
  }


  cat("End of test BenchMark EstimateWuForCenterdSample","\n")
